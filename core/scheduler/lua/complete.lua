...
if redis.call('HGET',KEYS[1],'cancel_requested') == '1' then
  ...
  redis.call('XACK',KEYS[2],ARGV[2],ARGV[3]);redis.call('XDEL',KEYS[2],ARGV[3]);redis.call('ZREM',KEYS[3],id)
  if tonumber(ARGV[5]) > 0 then redis.call('PEXPIRE',KEYS[1],ARGV[5]) else redis.call('DEL',KEYS[1]) end
  return 2
end
...
if tonumber(ARGV[4]) > 0 then redis.call('PEXPIRE',KEYS[1],ARGV[4]) else redis.call('DEL',KEYS[1]) end
return 1
