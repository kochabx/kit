...
func (e *Etcd) connect() error {
	client, err := clientv3.New(clientv3.Config{
		...
		DialKeepAliveTime:    time.Duration(e.config.KeepAliveTime) * time.Second,
		DialKeepAliveTimeout: time.Duration(e.config.KeepAliveTimeout) * time.Second,
		AutoSyncInterval:     time.Duration(e.config.AutoSyncInterval) * time.Second,
